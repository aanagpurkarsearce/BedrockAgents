"""This module contains a helper class for building and using Amazon Bedrock Agents. 
...
the class:
    ...
    >>> agents-and-function-calling = AgentsForAmazonBedrock()
    >>> name = "my_agent"
    >>> descr = "my agent description"
    >>> instructions = "you are an agent that ..."
    >>> model_id = "...haiku..."
    >>> agent_id = agents-and-function-calling.create_agent(name, descr, instructions, model_id)
    >>>
    >>> action_group_name = "my_action_group"
    >>> action_group_descr = "my action group description"
    >>> lambda_code = "my_lambda.py"
    >>> function_defs = [{ ... }]
    >>> action_group_arn = agents-and-function-calling.add_action_group_with_lambda(agent_id,
                                         lambda_function_name, lambda_code, 
                                         function_defs, action_group_name, action_group_descr)
    >>> agents-and-function-calling.simple_agent_invoke("when's my next payment due?", agent_id)
...
class AgentsForAmazonBedrock:
    """Provides an easy to use wrapper for Amazon Bedrock Agents.
    """
    ...
    def _create_lambda_iam_role(self, agent_name: str,
        ...
        # create a policy to allow Lambda to invoke sub-agents-and-function-calling and look up info about each sub-agent.
    ...
    def create_lambda(self, 
        ...
        Args:
            ...
            sub_agent_arns (List[str]): List of ARNs of the sub-agents-and-function-calling that this Lambda is allowed to invoke.
    ...
    def add_action_group_with_lambda(self,
        ...
        Args:
            ...
            sub_agent_arns (List[str], Optional): list of ARNs of sub-agents-and-function-calling (if any) to permit the Lambda to invoke
        """
    ...
    def get_function_defs(self, agent_name: str) -> List[dict]:
        ...
        """
        # "sub-agents-and-function-calling-ag"
        _agent_id = self.get_agent_id_by_name(agent_name)
        if _agent_id is None:
            raise ValueError(f"Agent {agent_name} not found")
    ...
    def create_supervisor_agent(self, supervisor_agent_name: str, 
        ...
        available sub-agents-and-function-calling. For each sub-agent, the supervisor agent has a distinct action in its action group.
        ...
        Args:
            ...
            sub_agent_names (List[str]): A list of named sub-agents-and-function-calling that this supervisor agent uses to get its work done.
        ...
        To accomplish those tasks, you delegate your work to a sub-agent, but you never reveal to the user that you are using sub-agents-and-function-calling.
        ...
        Here is your list of sub-agents-and-function-calling: """
        ...
        for _agent_name in sub_agent_names:
            ...
            # to know what expert agents-and-function-calling it has available.
        ...
        self.add_action_group_with_lambda( supervisor_agent_name,
                                     ...
                                     _function_defs,
                                     "sub-agents-and-function-calling-ag",
                                     "set of actions that invoke sub-agents-and-function-calling",
                                     _sub_agent_arns)
